classdef FLW_segmentation_Loose<CLW_generic
    properties
        FLW_TYPE=3;
        h_code_list;
        h_code_btn;
    ...
    % GUI part ------------------------------------
    methods
        function obj = FLW_segmentation_Loose(batch_handle)
        ...
        function GUI_update(obj,batch_pre)
            ...
            % Get all types of events code.
            st=sort(unique({lwdataset(1).header.events.code}));
            for dataset_pos=2:length(lwdataset)
                st= union(st,{lwdataset(dataset_pos).header.events.code});
            end
            if isempty(st)
                error('***No event code in the datasets.***')
            end
